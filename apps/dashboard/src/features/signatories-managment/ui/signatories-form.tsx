...
import { Loader2, Save, UserCheck, Upload, X, ImageIcon } from "lucide-react";
import {
    useSignatories,
    useUpdateSignatory,
    SIGNATORY_ROLES,
...
function SignatoryCard({ role, initialName, initialIdNumber, initialSignatureImage }: SignatoryCardProps) {
    const [name, setName] = useState(initialName);
    const [idNumber, setIdNumber] = useState(initialIdNumber);
    // selectedFile: File to upload | null to clear | undefined = no change
    const [selectedFile, setSelectedFile] = useState<File | null | undefined>(undefined);
    ...
    useEffect(() => { setIdNumber(initialIdNumber); }, [initialIdNumber]);
    useEffect(() => {
        ...
        <div className="rounded-xl border bg-card p-5 space-y-4 shadow-sm transition-shadow hover:shadow-md">
            ...
            <div className="grid gap-3">
                <div className="space-y-1.5">
                    <Label htmlFor={`name-${role}`} className="text-xs">Nombre completo</Label>
                    <Input
                        id={`name-${role}`}
                        value={name}
                ...
                <div className="space-y-1.5">
                    <Label htmlFor={`ci-${role}`} className="text-xs">Cédula de identidad</Label>
                    <Input
                        id={`ci-${role}`}
                        value={idNumber}
                    ...
                    >
                        {hasClearedImage ? (
                            <>
                                ...
                                <p className="text-xs">Firma eliminada — sube una nueva o guarda sin firma</p>
                            </>
                        ) : (
                            <>
                                <Upload className="h-5 w-5" />
                                <p className="text-xs font-medium">Arrastra o haz clic para subir</p>
                                <p className="text-[10px] text-muted-foreground">PNG o JPG · máx. 500 KB</p>
                            </>
                        )}
                    </div>
                )}
...
export function SignatoriesForm() {
    ...
    const sigMap = Object.fromEntries(signatories.map((s) => [s.role, s])) as Record<
        SignatoryRole,
        (typeof signatories)[number] | undefined
    >;
